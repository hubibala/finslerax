...
class WeinrebModel(eqx.Module):
    ...
    def __init__(self, key, dim=50):
        ...
        self.metric = EnergyBasedRanders(self.manifold, self.ebm, wind_scale=1.0, beta=100.0)
        self.solver = AVBDSolver(step_size=0.5, grad_clip=1.0, iterations=100, implicit_diff=True)
...
def main():
    ...
    subsample = 5
    plt.scatter(dataset.X[::subsample, 0], dataset.X[::subsample, 1], 
                s=2, c='white', alpha=0.3, label='Weinreb Data (PC1/PC2)')
                
    # Quiver plot for wind
    stride = 3
    plt.quiver(X_grid[::stride, ::stride], Y_grid[::stride, ::stride], 
               W_x[::stride, ::stride], W_y[::stride, ::stride], 
               color='red', alpha=0.7, scale=10, width=0.003, label='Wind $-\\nabla E(x)$')
    ...
    if pairs is not None and len(pairs) > 0:
        ...
        for idx, (p1, p2) in enumerate(sample_pairs):
            ...
            traj = trained_model.solver.solve(trained_model.metric, z_start, z_end, n_steps=100)
            ...
            # Plot
            if idx == 0:
                plt.plot(path_pc1, path_pc2, 'm-', linewidth=2, label='Computed 50D Geodesic')
            else:
                plt.plot(path_pc1, path_pc2, 'm-', linewidth=2)
                
            plt.scatter(z_start[0], z_start[1], c='cyan', s=50, marker='o', edgecolors='black', zorder=5)
            plt.scatter(z_end[0], z_end[1], c='magenta', s=50, marker='X', edgecolors='black', zorder=5)
