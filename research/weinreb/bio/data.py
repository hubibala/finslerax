...
class DataLoader:
    ...
    def preprocess(self, n_top_genes: int = 2000, pca_components: int = 50):
        if self.mode == "real" and self.adata is not None:
            ...
            if (
                "X_pca" not in self.adata.obsm
                or self.adata.obsm["X_pca"].shape[1] != pca_components
            ):
    ...
    def extract_lineage_pairs(self) -> jnp.ndarray:
        ...
        if time_col:
            print(
                f">>> NO CLONES FOUND. Falling back to heuristic: '{time_col}' neighbors."
            )
            return self._extract_pseudotime_pairs(time_col)

        return None

    def _extract_pseudotime_pairs(
        self, time_col: str, n_pairs: int = 5000
    ) -> jnp.ndarray:
        ...
        sample_idxs = np.random.choice(
            valid_indices, min(len(valid_indices), 10000), replace=False
        )
    ...
    def get_jax_data(self, use_pca: bool = True) -> BioDataset:
        if self.mode != "real" or self.adata is None:
            N = 100
            return BioDataset(
                jnp.zeros((N, 50)), jnp.zeros((N, 50)), jnp.zeros(N), None
            )
