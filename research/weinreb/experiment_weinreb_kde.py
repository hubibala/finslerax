...
class KDEWeinrebModel(eqx.Module):
    ...
    def __init__(self, centers, sigma=1.0, dim=5):
        ...
        self.metric = EnergyBasedRanders(self.manifold, self.ebm, wind_scale=0.1, beta=10.0)
        self.solver = AVBDSolver(step_size=0.05, grad_clip=1.0, iterations=200, implicit_diff=True)
...
def main():
    ...
    if pairs is not None and len(pairs) > 0:
        ...
        for idx, (p1, p2) in enumerate(sample_pairs):
            ...
            if idx == 0:
                plt.plot(path_pc1, path_pc2, 'm-', linewidth=2, label=f'Computed {dim}D Geodesic')
            else:
