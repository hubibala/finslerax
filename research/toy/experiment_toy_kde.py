...
from ham.solvers.geodesic_learning import GeodesicLearningSolver
...
class KDEWeinrebModel(eqx.Module):
    ...
    def __init__(self, centers, sigma=1.0, dim=2):
        ...
        self.metric = EnergyBasedRanders(self.manifold, self.ebm, wind_scale=0.15, beta=10.0)
        self.solver = AVBDSolver(step_size=0.5, grad_clip=1.0, iterations=200, implicit_diff=True)
...
def main():
    ...
    plt.scatter(centers[:, 0], centers[:, 1], 
                s=5, c='white', alpha=0.5, label='Synthetic Data')
                
    stride = 3
    plt.quiver(X_grid[::stride, ::stride], Y_grid[::stride, ::stride], 
               W_x[::stride, ::stride], W_y[::stride, ::stride], 
               color='red', alpha=0.7, scale=15, width=0.003, label='Wind $-\\nabla E(x)$')
    ...
    def solve_avbd(z_start, z_end):
        return model.solver.solve(model.metric, z_start, z_end, train_mode=False, n_steps=200)
    ...
    for idx, (z_start, z_end, label) in enumerate(pairs):
        ...
        # Plot AVBD
        plt.plot(traj_avbd.xs[:, 0], traj_avbd.xs[:, 1], color=colors[idx], 
                 linewidth=3, linestyle='-', label=f'AVBD: {label}')
                 
        plt.scatter(z_start[0], z_start[1], c=colors[idx], s=100, marker='o', edgecolors='black', zorder=5)
        plt.scatter(z_end[0], z_end[1], c=colors[idx], s=100, marker='X', edgecolors='black', zorder=5)
