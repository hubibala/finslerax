...
class ToyModel(eqx.Module):
    ...
    def __init__(self, key):
        ...
        self.metric = EnergyBasedRanders(
            self.manifold, self.ebm, wind_scale=1.0, beta=100
        )
        self.solver = AVBDSolver(
            step_size=0.5, iterations=100, tol=1e-6, implicit_diff=True
        )
...
def main():
    ...
    plt.scatter(
        dataset.X[:, 0], dataset.X[:, 1], c="white", s=5, alpha=0.5, label="Data"
    )
    ...
    traj_fwd = trained_model.solver.solve(
        trained_model.metric, pt_root, pt_b1, n_steps=10
    )
    traj_bwd = trained_model.solver.solve(
        trained_model.metric, pt_b1, pt_root, n_steps=10
    )

    # Tricky geodesic between branches
    traj_tricky = trained_model.solver.solve(
        trained_model.metric, pt_b1, pt_b2, n_steps=20
    )

    plt.plot(
        traj_fwd.xs[:, 0], traj_fwd.xs[:, 1], "r-", linewidth=2, label="Fwd (Root->B1)"
    )
    plt.plot(
        traj_bwd.xs[:, 0], traj_bwd.xs[:, 1], "g--", linewidth=2, label="Bwd (B1->Root)"
    )
    plt.plot(
        traj_tricky.xs[:, 0],
        traj_tricky.xs[:, 1],
