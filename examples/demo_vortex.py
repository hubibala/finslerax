...
def main():
    ...
    w_net = vortex_field(
        vortex_center, strength=10.0, decay=5.0
    )  # Very strong wind to force curvature
    ...
    plot_trajectory(
        ...
        linestyle="--",
        linewidth=2,
        label="Stiff Solver (beta=20)",
    )
    plot_trajectory(
        ax, traj_relaxed, color="red", linewidth=2, label="Relaxed Solver (beta=0.5)"
    )
