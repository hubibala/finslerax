...
**Usage**::
    ...
    python examples/experiment_wildfire_geometric.py \\
        --checkpoint results/phaseW1/metric_seed0.eqx \\
...
def plot_fire_corridors(paths, arc_lengths, arrival_field, domain_shape,
    ...
    for ext in ("pdf", "png"):
        fig.savefig(os.path.join(output_dir, f"phaseW3_corridors{suffix}.{ext}"))
    plt.close(fig)
    print(f"  Saved: phaseW3_corridors{suffix}.pdf/png")
...
def plot_jacobi_divergence(divergence, output_dir, suffix=""):
    ...
    os.makedirs(output_dir, exist_ok=True)
    for ext in ("pdf", "png"):
        fig.savefig(os.path.join(output_dir, f"phaseW3_jacobi{suffix}.{ext}"))
    plt.close(fig)
    print(f"  Saved: phaseW3_jacobi{suffix}.pdf/png")
...
def plot_curvature_field(kappa, grid_xy, domain_shape, output_dir, suffix=""):
    ...
    os.makedirs(output_dir, exist_ok=True)
    for ext in ("pdf", "png"):
        fig.savefig(os.path.join(output_dir, f"phaseW3_curvature{suffix}.{ext}"))
    plt.close(fig)
    print(f"  Saved: phaseW3_curvature{suffix}.pdf/png")
...
def run_geometric_analysis(metric, solver, source_world, scenario, domain_shape,
    ...
    if compute_curvature:
        ...
        print("  [W3.3] Curvature skipped (--no_curvature flag).")
...
def main():
    ...
    print("=" * 65)
    print("Phase W3: Wildfire Geometric Analysis")
    print("=" * 65)
