...
def main():
    ...
    plot_vector_field(
        ax, grid_pts, vecs_true, color="cyan", scale=0.15, alpha=0.3, label="Truth"
    )
    plot_vector_field(
        ax,
        grid_pts,
        ...
        scale=0.15,
        alpha=0.9,
        label="Learned (Smoothed)",
    )
    # Training set
    cos_sim_train = jnp.mean(
        jnp.sum(vecs_true * vecs_pred, axis=-1)
        / (
            jnp.linalg.norm(vecs_true, axis=-1) * jnp.linalg.norm(vecs_pred, axis=-1)
            + 1e-8
        )
    )
    ...
    # Held-out set
    X_test = jax.vmap(sphere.random_sample, in_axes=(0, None))(
    ...
    cos_sim_test = jnp.mean(
        ...
        / (
            jnp.linalg.norm(V_test_true, axis=-1)
            * jnp.linalg.norm(V_test_pred, axis=-1)
            + 1e-8
        )
    )
