...
**HAMTools spec references:**
    ...
    spec/ARCH_SPEC.md § 3 (CovariateMeshRanders), § 4 (AVBDSolver)

Usage::

    # Synthetic smoke-test (no dataset required):
    python examples/experiment_wildfire_mesh.py --synthetic --quick

    # Real data (one scene):
    python examples/experiment_wildfire_mesh.py \\
...
def get_config(quick: bool = False) -> dict:
    ...
    return dict(
        ...
        avbd_n_steps=10 if quick else 40,
...
def _predict_arrivals_mesh_chunked(
    ...
    Args:
        ...
        solver:          AVBD solver.
        source_3d:       Ignition world coordinate, shape ``(3,)``.
        eval_pixels:     Shape ``(N, 2)`` int — ``[row, col]`` pairs.
        elev_raster:     Shape ``(H, W)`` elevation array for Z-coordinate lookup.
        pixel_spacing_m: Metres per pixel.
        n_steps:         AVBD path discretisation steps.
        chunk_size:      Pixels to vmap over simultaneously.

    Returns:
        Shape ``(N,)`` float64 — predicted geodesic arc lengths.
    """
    def _single_arrival(x3d: jax.Array) -> jax.Array:
        traj = solver.solve(bound_metric, source_3d, x3d, n_steps=n_steps)
        path = traj.xs                              # (n_steps+1, 3)
        segs = jnp.diff(path, axis=0)              # (n_steps, 3)
        mids = (path[:-1] + path[1:]) * 0.5        # (n_steps, 3)
...
def train_scene_mesh(
    ...
    for epoch in range(cfg["n_epochs"]):
        ...
        val_rs = [
            _val_pearson_r_mesh(
                metric, solver, sc, mesh, face_cov_5, face_fuel_codes, cfg
            )
            for sc in val_scens
        ]
...
def run_synthetic_mesh(cfg: dict, output_dir: str, use_wind: bool = True) -> list:
    ...
    for slope_level in slope_levels:
        ...
        pred = _predict_arrivals_mesh_chunked(
            bound, solver, source_3d, eval_pixels,
