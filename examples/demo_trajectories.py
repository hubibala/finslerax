...
def pure_advection(
    p0: jnp.ndarray, flow: Callable, dt: float = 0.005, n_steps: int = 800
) -> Trajectory:
    """
...
def path_length(trajectory: Union[jnp.ndarray, NamedTuple, tuple]) -> float:
    ...
    if isinstance(trajectory, (tuple, NamedTuple)):
        if hasattr(trajectory, "xs"):
            ...
            raise ValueError(
                f"Cannot extract position array from trajectory: {type(trajectory)}"
            )
    else:
        xs = trajectory
...
def main():
    ...
    wind_flow = rossby_haurwitz(
        R=4, omega=1.0, K=0.8
    )  # K!=omega avoids stagnation at equator
    ...
    print("\nResults:")
    print(
        f"  Optimal BVP path (red)    → Randers energy: {traj_bvp.energy:>8.4f}   | Euclidean length: {path_length(traj_bvp):>6.4f}"
    )
    print(
        f"  Passive drift (orange)    → Euclidean length: {path_length(traj_passive):>6.4f}"
    )
    print(
        f"  Verification shot (green) → Euclidean length: {path_length(traj_verif):>6.4f}"
    )
    ...
    overlap_dev = jnp.mean(
        jnp.linalg.norm(xs_bvp[:min_len] - xs_verif[:min_len], axis=1)
    )
    ...
    # Trajectories
    plot_trajectory(
        ax, traj_bvp, color="red", linewidth=4.0, label="Optimal Randers geodesic (BVP)"
    )
    plot_trajectory(
        ax,
        traj_passive,
        ...
        linewidth=2.5,
        linestyle="--",
        label="Pure passive advection (dx/dt = wind)",
    )
    plot_trajectory(
        ax,
        traj_verif,
        ...
        linewidth=2.8,
        linestyle="-",
        label="Geodesic shot from BVP initial velocity",
    )

    # Indicatrices (optional)
    plot_indicatrices(
        ax, metric, traj_bvp.xs[::6], color="purple", alpha=0.6, scale=0.12
    )
