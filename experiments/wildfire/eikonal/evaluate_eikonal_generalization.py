...
def main():
    ...
    for eval_scene_short in scenes:
        ...
        for sid, eid in test_list[:10]:
            scen = load_wildfire_scenario(
                loader, sid, eid, normalizer, k_train_obs=1000, seed=42
            )
    ...
    for source_scene in scenes:
        for eval_scene in scenes:
            ...
            for m_idx, m in enumerate(models[source_scene]):
                ...
                print(
                    f"  Model seed {m_idx}: Corr = {avg_corr:.3f}, IoU@50 = {avg_iou:.3f}, RelRMSE = {avg_rrmse:.3f}, MAE = {avg_mae:.3f}"
                )
    ...
    def safe_std(arr):
        return np.nanstd(arr) if len(arr) > 0 else 0.0
    ...
    for idx, (label, (key, mean_fn, std_fn)) in enumerate(metrics_to_plot.items()):
        means = [
            mean_fn(results["In-Distribution"][key]),
            mean_fn(results["Out-of-Distribution"][key]),
        ]
        stds = [
            std_fn(results["In-Distribution"][key]),
            std_fn(results["Out-of-Distribution"][key]),
        ]
    ...
    def get_physical_features(m_bound, indices, cols, spacing, origin):
        def get_one(idx):
            ...
            col = idx % cols
            x_world = jnp.array(
                [col * spacing + origin[0], row * spacing + origin[1]],
                dtype=jnp.float32,
            )
