...
from experiments.wildfire.eikonal.experiment_eikonal_flat import get_config, make_metric, make_solver, bind_scenario_to_metric, _ignition_to_world
...
def main():
    ...
    parser.add_argument("--data_root", type=str, required=True, help="Path to the real fire dataset")
    parser.add_argument("--checkpoint", type=str, required=True, help="Path to the trained .eqx checkpoint")
    parser.add_argument("--scene_id", type=str, default="0001", help="Scene ID to evaluate on")
    parser.add_argument("--train_data_root", type=str, default=None, help="Path to original train data (to fit Normalizer). Defaults to data_root.")
    ...
    try:
        ...
        mask_dir = os.path.join(train_data_root, train_scene_id, "Satellite_Images_Mask")
        if not os.path.exists(mask_dir):
            mask_dir = os.path.join(train_data_root, "Satellite_Images_Mask")
        event_ids = [d for d in sorted(os.listdir(mask_dir)) if os.path.isdir(os.path.join(mask_dir, d))]
    ...
    print(f"Fitting normalizer on {len(train_list)} training fires from {train_scene_id}...")
    raw_train = []
    ...
    try:
        ...
        eval_event_ids = [d for d in sorted(os.listdir(mask_dir)) if os.path.isdir(os.path.join(mask_dir, d))]
