...
class E1_SolverForwardRuntime(Experiment):
    ...
    def __init__(self, grid_sizes: List[int] = None):
        super().__init__()
        self.grid_sizes = grid_sizes or [
            20,
            40,
            60,
        ]  # Kept small because AVBD is slow for large N
    ...
    def run(self) -> ExperimentResult:
        ...
        for N in self.grid_sizes:
            ...
            # Warmup Eikonal
            _, _, _ = solver_eik.solve(
                metric, source_coords, (0, N - 1, 0, N - 1), (N, N)
            )

            start = time.time()
            T_eik, _, _ = solver_eik.solve(
                metric, source_coords, (0, N - 1, 0, N - 1), (N, N)
            )
            ...
            print(
                f"    Eikonal: {time_eik:.4f}s, AVBD (est): {time_avbd_est:.4f}s, Speedup: {results[-1]['speedup']:.1f}x"
            )
        ...
        return ExperimentResult(
            ...
            arrays={
                "grid_sizes": np.array([r["N"] for r in results]),
    ...
    def visualize(self, save_path: Optional[str] = None) -> plt.Figure:
        ...
        axes[0].loglog(
            N, t_avbd, "s-", label="AVBD (Geodesic BVP - est)", markersize=10
        )
...
class E3_RegularizationStrategies(Experiment):
    ...
    def run(self) -> ExperimentResult:
        ...
        for solver_name in ["eikonal", "avbd"]:
            for reg_type in ["none", "tv", "tikhonov"]:
                ...
                T_pred, _, _ = solver.solve(
                    metric_rec, source_coords, (0, N - 1, 0, N - 1), (N, N)
                )
    ...
    def visualize(self, save_path: Optional[str] = None) -> plt.Figure:
        ...
        for i, reg_type in enumerate(["none", "tv", "tikhonov"]):
            ...
            axes[0, i + 1].imshow(
                res_eik["G"][0], origin="upper", cmap="viridis", vmin=vmin, vmax=vmax
            )
            axes[0, i + 1].set_title(
                f"Eikonal {reg_type.upper()} (T err={res_eik['error']:.3f})"
            )

            res_avbd = self.results[f"avbd_{reg_type}"]
            axes[1, i + 1].imshow(
                res_avbd["G"][0], origin="upper", cmap="viridis", vmin=vmin, vmax=vmax
            )
            axes[1, i + 1].set_title(
                f"AVBD {reg_type.upper()} (T err={res_avbd['error']:.3f})"
            )
...
class E5_Scalability(Experiment):
    ...
    def run(self) -> ExperimentResult:
        ...
        for N in self.grid_sizes:
            ...
            obs_mask = create_sparse_observation_mask(
                N, N, 50.0 / (N * N), source_mask, seed=59
            )
        ...
        return ExperimentResult(
            ...
            arrays={
                "N_sizes": np.array(self.grid_sizes),
                "M_sizes": np.array(self.M_sizes),
            },
            metadata={"grid_sizes": self.grid_sizes},
        )
    ...
    def visualize(self, save_path: Optional[str] = None) -> plt.Figure:
        ...
        axes[1].loglog(
            M, t_avbd, "s-", color="red", label="AVBD Step Time", markersize=8
        )
