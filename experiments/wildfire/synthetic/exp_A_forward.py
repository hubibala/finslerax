...
class A1_IsotropicConvergence(Experiment):
    ...
    def run(self) -> ExperimentResult:
        ...
        for N in self.grid_sizes:
            ...
            T_physical, X, Y = solver.solve(
                metric, source_coords, grid_extent=(0, L, 0, L), grid_shape=(N, N)
            )
    ...
    def visualize(self, save_path: Optional[str] = None) -> plt.Figure:
        ...
        ax.loglog(
            ...
            e_l2,
            "o-",
            label=f"L² (rate={self.result.metrics['rate_l2']:.2f})",
            markersize=8,
        )
        ax.loglog(
            h,
            e_linf,
            "s-",
            label=f"L∞ (rate={self.result.metrics['rate_linf']:.2f})",
            markersize=8,
        )
...
class A2_UniformAnisotropic(Experiment):
    ...
    def run(self) -> ExperimentResult:
        ...
        T_exact = anisotropic_distance_field(
            N, N, source_i, source_j, self.a**2, self.b**2, 0.0
        )
...
class A3_RotatedAnisotropic(Experiment):
    ...
    def run(self) -> ExperimentResult:
        ...
        T_exact = anisotropic_distance_field(
            N, N, source_i, source_j, self.lambda1, self.lambda2, self.theta
        )
...
class A5_CombinedAnisotropicDrift(Experiment):
    ...
    def run(self) -> ExperimentResult:
        ...
        T_fine, _, _ = solver.solve(
            metric_fine,
            source_coords_fine,
            grid_extent=(0, N_fine - 1, 0, N_fine - 1),
            grid_shape=(N_fine, N_fine),
        )
        ...
        rel_l2 = float(
            jnp.sqrt(jnp.mean(diff**2, where=interior))
            / jnp.mean(jnp.abs(T), where=interior)
        )
...
class A6_PiecewiseMetric(Experiment):
    ...
    def visualize(self, save_path: Optional[str] = None) -> plt.Figure:
        ...
        axes[0].axvline(
            self.N // 2, color="r", linestyle="--", linewidth=2, label="Interface"
        )
        axes[0].legend()
...
class A7_SpatiallyVaryingMetric(Experiment):
    ...
    def visualize(self, save_path: Optional[str] = None) -> plt.Figure:
        ...
        axes[2].set_title(
            f"Eikonal Residual (mean={self.result.metrics['mean_residual']:.4f})"
        )
...
class A8_MultiSource(Experiment):
    ...
    def visualize(self, save_path: Optional[str] = None) -> plt.Figure:
        ...
        for ax, T_arr, title in [
            (axes[0], self.T, "Computed"),
            (axes[1], self.T_exact, "Exact"),
        ]:
...
class A9_IterationComplexity(Experiment):
    ...
    def run(self) -> ExperimentResult:
        ...
        for N in self.grid_sizes:
            ...
            # Warmup JIT
            T_warmup, _, _ = solver.solve(
                metric,
                source_coords,
                grid_extent=(0, N - 1, 0, N - 1),
                grid_shape=(N, N),
            )
            jax.block_until_ready(T_warmup)

            start = time.time()
            T_timed, _, _ = solver.solve(
                metric,
                source_coords,
                grid_extent=(0, N - 1, 0, N - 1),
                grid_shape=(N, N),
        ...
        return ExperimentResult(
            ...
            arrays={"grid_sizes": np.array(self.grid_sizes), "times": np.array(times)},
            metadata={"grid_sizes": self.grid_sizes},
        )
