...
class D1_TerrainDriven(Experiment):
    ...
    def run(self) -> ExperimentResult:
        ...
        elevation = 50 * jnp.exp(
            -((I - N / 2) ** 2 + (J - N / 2) ** 2) / (2 * (N / 4) ** 2)
        )
    ...
    def visualize(self, save_path: Optional[str] = None) -> plt.Figure:
        ...
        for i in range(paths_np.shape[0]):
            axes[1].plot(
                paths_np[i, :, 1], paths_np[i, :, 0], "w--", linewidth=2, alpha=0.8
            )
...
class D2_WindDriven(Experiment):
    ...
    def __init__(
        self, N: int = 150, wind_speed: float = 0.25, wind_dir_deg: float = 45
    ):
        super().__init__()
        self.N = N
    ...
    def run(self) -> ExperimentResult:
        ...
        T_nowind, _, _ = solver.solve(
            metric_nowind, source_coords, (0, N - 1, 0, N - 1), (N, N)
        )
        ...
        return ExperimentResult(
            ...
            metadata={
                ...
                "wind_dir_deg": np.rad2deg(self.wind_dir),
            },
        )
...
class D4_CombinedScenario(Experiment):
    ...
    def run(self) -> ExperimentResult:
        ...
        elevation = 30 * jnp.sin(2 * np.pi * I / N) * jnp.cos(
            np.pi * J / N
...
class D5_FireLineReconstruction(Experiment):
    ...
    def run(self) -> ExperimentResult:
        ...
        opt_eik = MetricRecoveryOptimizer(
            ...
            lambda_H=0.01,
            lambda_W=0.01,
            constrain_isotropic=True,
        )
        opt_eik.fit(
            source_coords, T_true, obs_mask, n_iter=self.n_iter, lr=0.05, verbose=True
        )
        ...
        T_pred_eik, _, _ = solver.solve(
            metric_eik, source_coords, (0, N - 1, 0, N - 1), (N, N)
        )
        T_pred_avbd, _, _ = solver.solve(
            metric_avbd, source_coords, (0, N - 1, 0, N - 1), (N, N)
        )
        ...
        self.T_true, self.T_pred_eik, self.T_pred_avbd, self.obs_mask = (
            T_true,
            T_pred_eik,
            T_pred_avbd,
            obs_mask,
        ...
        return ExperimentResult(
            ...
            arrays={
                ...
                "T_eik": np.array(T_pred_eik),
                "T_avbd": np.array(T_pred_avbd),
            },
            metadata={"N": N},
        )
    ...
    def visualize(self, save_path: Optional[str] = None) -> plt.Figure:
        ...
        plot_arrival_time(
            self.T_pred_eik,
            ax=axes[1],
            title=f"Eikonal Reconstruction (MAE={self.result.metrics['err_eikonal']:.2f})",
        )
        plot_arrival_time(
            self.T_pred_avbd,
            ax=axes[2],
            title=f"AVBD Reconstruction (MAE={self.result.metrics['err_avbd']:.2f})",
        )
...
class D6_ParameterSensitivity(Experiment):
    ...
    def run(self) -> ExperimentResult:
        ...
        for pert in self.perturbation_levels:
            ...
            T_pert, _, _ = solver.solve(
                metric_pert, source_coords, (0, N - 1, 0, N - 1), (N, N)
            )

            interior = get_interior_mask(N, N, 5, source_mask)
            rel_err = float(
                jnp.sqrt(jnp.mean((T_true[interior] - T_pert[interior]) ** 2))
                / jnp.abs(T_true[interior]).max()
            )
        ...
        return ExperimentResult(
            ...
            success=True,
            metrics={
                "error_at_10pct": results[2]["T_error"],
                "error_at_30pct": results[4]["T_error"],
            },
            arrays={
                "perturbations": np.array([r["perturbation"] for r in results]),
