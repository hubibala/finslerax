...
class ExperimentResult:
    ...
    def save(self, base_dir: str = None):
        ...
        summary = {
            ...
            'metrics': {k: float(v) if isinstance(v, (np.floating, float, jax.Array)) else v 
...
class Experiment(ABC):
    ...
    def execute(self, save: bool = True, visualize: bool = True, 
...
def _bilinear_interp_point(grid: jax.Array, pt_pixel: jax.Array) -> jax.Array:
    ...
    return (grid[:, y0, x0] * (1.0 - fx) * (1.0 - fy) +
...
class SyntheticZermeloMetric(AsymmetricMetric):
    ...
    def __init__(self, H_grid: jax.Array, W_grid: jax.Array, lam_grid: Optional[jax.Array] = None):
...
def anisotropic_distance_field(M: int, N: int, source_i: int, source_j: int,
                                lambda1: float, lambda2: float, theta: float = 0.0) -> jax.Array:
    """Compute exact distance for anisotropic metric."""
    Y, X = jnp.meshgrid(jnp.arange(M) - source_i, jnp.arange(N) - source_j, indexing='ij')
...
def create_metric_from_eigenvalues(M: int, N: int, lambda1: float, lambda2: float,
                                    theta: float = 0.0) -> jax.Array:
...
def compute_errors(T_computed: jax.Array, T_exact: jax.Array, 
                   mask: Optional[jax.Array] = None) -> Dict[str, float]:
    ...
    def calc_errors():
        ...
        return {'l1': float(l1), 'l2': float(l2), 'linf': float(linf), 'rel_l2': float(rel_l2), 'rel_linf': float(rel_linf)}
    
    def nan_errors():
        return {'l1': float('nan'), 'l2': float('nan'), 'linf': float('nan'), 'rel_l2': float('nan'), 'rel_linf': float('nan')}
...
def compute_convergence_rate(h_values: List[float], errors: List[float]) -> Tuple[float, float]:
...
def plot_arrival_time(T: np.ndarray, ax: plt.Axes = None, title: str = "Arrival Time",
                      contour_levels: int = 20, cmap: str = 'viridis') -> plt.Axes:
...
def plot_error_map(error: np.ndarray, ax: plt.Axes = None, title: str = "Error",
                   cmap: str = 'RdBu_r', symmetric: bool = True) -> plt.Axes:
    """Plot error field."""
    if ax is None:
        fig, ax = plt.subplots(figsize=(8, 6))
...
def plot_metric_ellipses(G: np.ndarray, ax: plt.Axes, step: int = 10, 
                         scale: float = 0.4, color: str = 'blue') -> None:
    ...
    for i in range(step//2, M, step):
        for j in range(step//2, N, step):
            ...
            try:
                ...
                ellipse = Ellipse((j, i), width=2*rx, height=2*ry, angle=angle,
                                  fill=False, edgecolor=color, linewidth=0.5, alpha=0.6)
                ax.add_patch(ellipse)
            except:
                pass

def plot_drift_field(W: np.ndarray, ax: plt.Axes, step: int = 10, 
                     color: str = 'red', scale: float = None) -> None:
...
def get_interior_mask(M: int, N: int, boundary_width: int = 3, 
                      source_mask: Optional[jax.Array] = None, source_radius: int = 3) -> jax.Array:
    """Create mask for interior points."""
    mask = jnp.zeros((M, N), dtype=bool)
    mask = mask.at[boundary_width:-boundary_width, boundary_width:-boundary_width].set(True)
    ...
    if source_mask is not None:
        ...
        def dist_to_sources(i, j):
            dists = jnp.where(source_mask, jnp.maximum(jnp.abs(I - i), jnp.abs(J - j)), 1000)
...
def create_sparse_observation_mask(M: int, N: int, fraction: float,
