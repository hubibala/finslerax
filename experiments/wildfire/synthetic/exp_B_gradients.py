...
from experiments.wildfire.synthetic.experiment_base import (
    Experiment,
    ExperimentResult,
    create_metric_from_eigenvalues,
    plot_arrival_time,
    register_experiment,
)
...
@functools.partial(jax.jit, static_argnames=("hx", "hy", "max_iters", "tol"))
def batched_fd_G(
    G, B, source_mask, dL_dT, pts_c, pts_i, pts_j, eps, hx, hy, max_iters, tol
):
    def single_fd(c, i, j):
        G_plus = G.at[c, i, j].add(eps)
        G_minus = G.at[c, i, j].add(-eps)
        L_plus = compute_loss_jitted(
            G_plus, B, source_mask, dL_dT, hx, hy, max_iters, tol
        )
        L_minus = compute_loss_jitted(
            G_minus, B, source_mask, dL_dT, hx, hy, max_iters, tol
        )
...
def batched_fd_B(
    ...
    def single_fd(c, i, j):
        ...
        L_plus = compute_loss_jitted(
            G, B_plus, source_mask, dL_dT, hx, hy, max_iters, tol
        )
        L_minus = compute_loss_jitted(
            G, B_minus, source_mask, dL_dT, hx, hy, max_iters, tol
        )
        return (L_plus - L_minus) / (2 * eps)
...
class B1_FD_Isotropic(Experiment):
    ...
    def run(self) -> ExperimentResult:
        ...
        fd_G_all, fd_B_all = evaluate_fd_gradients(
            G, B, source_mask, dL_dT, test_points, self.eps
        )
        ...
        pct_good_G = (
            sum(1 for e in G_errors if e < 0.1) / len(G_errors) * 100 if G_errors else 0
        )
        pct_good_B = (
            sum(1 for e in B_errors if e < 0.1) / len(B_errors) * 100 if B_errors else 0
        )
        ...
        return ExperimentResult(
            ...
            metrics={
                "pct_good_G": pct_good_G,
                "pct_good_B": pct_good_B,
                "mean_err_G": float(np.mean(G_errors)),
                "mean_err_B": float(np.mean(B_errors)),
            },
            arrays={
                "G_fd": np.array([r["fd"] for r in results_G]),
    ...
    def visualize(self, save_path: Optional[str] = None) -> plt.Figure:
        ...
        for ax, results, name in [
            (axes[0, 0], self.results_G, "G"),
            (axes[0, 1], self.results_B, "B"),
        ]:
...
class B2_FD_Anisotropic(Experiment):
    ...
    def run(self) -> ExperimentResult:
        ...
        results = {0: [], 1: [], 2: []}
        fd_G_all, _ = evaluate_fd_gradients(
            G, B, source_mask, dL_dT, test_points, self.eps, G_channels=[0, 1, 2]
        )
        ...
        for c, name in [(0, "g11"), (1, "g12"), (2, "g22")]:
            ...
            if errs:
                metrics[f"pct_good_{name}"] = (
                    sum(1 for e in errs if e < 0.1) / len(errs) * 100
                )
                print(f"  {name}: {metrics[f'pct_good_{name}']:.1f}% < 10% err")

        self.results = results
        pct_good = {
            c: 100 * sum(1 for r in results[c] if r["rel_err"] < 0.1) / len(results[c])
            for c in [0, 1, 2]
        }
        return ExperimentResult(
            name=self.name,
            ...
            arrays={
                **{
                    f"{n}_fd": np.array([r["fd"] for r in results[c]])
                    for c, n in [(0, "g11"), (1, "g12"), (2, "g22")]
                },
                **{
...
class B3_FD_Drift(Experiment):
    ...
    def run(self) -> ExperimentResult:
        ...
        results = {0: [], 1: []}
        _, fd_B_all = evaluate_fd_gradients(
            G, B, source_mask, dL_dT, test_points, self.eps, B_channels=[0, 1]
        )
        ...
        return ExperimentResult(
            ...
            success=all(pct > 70 for pct in pct_good.values()),
            metrics={
                **{f"pct_good_{n}": pct_good[c] for c, n in [(0, "b1"), (1, "b2")]},
                **{
                    f"mean_err_{n}": float(np.mean([r["rel_err"] for r in results[c]]))
...
class B5_GradientDecay(Experiment):
    ...
    def run(self) -> ExperimentResult:
        ...
        I, J = np.meshgrid(
            np.arange(N) - target_i, np.arange(N) - target_j, indexing="ij"
        )
...
class B8_RandomDirections(Experiment):
    ...
    def run(self) -> ExperimentResult:
        ...
        for _ in range(self.n_directions):
            ...
            rel_err = (
                abs(fd_dir - impl_dir) / abs(fd_dir)
                if abs(fd_dir) > 1e-10
                else abs(impl_dir)
            )
...
class B9_GradientStability(Experiment):
    ...
    def run(self) -> ExperimentResult:
        ...
        for _ in range(self.n_perturbations):
            ...
            rel_diff = float(
                jnp.mean(jnp.abs(dL_dG_pert - dL_dG_base))
                / jnp.mean(jnp.abs(dL_dG_base))
            )
    ...
    def visualize(self, save_path: Optional[str] = None) -> plt.Figure:
        ...
        ax.axvline(
            np.mean(self.result.arrays["variations"]),
            color="r",
            linestyle="--",
