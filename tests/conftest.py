"""Pytest configuration for the finslerax test suite.

Numerical precision is controlled by JAX's native ``JAX_ENABLE_X64`` flag (read
at ``import jax``; see ``finslerax.utils.config``) — nothing is set here, so the suite
...
def pytest_report_header(config):
    ...
    return f"finslerax precision: {mode} [default dtype: {default_dtype()}]"
