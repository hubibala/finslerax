...
def compute_three_point_update(T1, T2, T3, Q11, Q22, Q33, Q12, Q13, Q23, B1, B2, B3, dx, dy, dz, eps=1e-12):
    vx, vy, vz = 1.0/dx, 1.0/dy, 1.0/dz
    c1 = T1*vx + B1
    c2 = T2*vy + B2
    c3 = T3*vz + B3
    
    a = (Q11*vx**2 + Q22*vy**2 + Q33*vz**2 + 
         2*Q12*vx*vy + 2*Q13*vx*vz + 2*Q23*vy*vz)
    
    b_half = -(vx * (Q11*c1 + Q12*c2 + Q13*c3) + 
               vy * (Q12*c1 + Q22*c2 + Q23*c3) + 
               vz * (Q13*c1 + Q23*c2 + Q33*c3))
    
    c = (c1 * (Q11*c1 + Q12*c2 + Q13*c3) + 
         c2 * (Q12*c1 + Q22*c2 + Q23*c3) + 
         c3 * (Q13*c1 + Q23*c2 + Q33*c3)) - 1.0
...
def voxel_update(T0, Tx, Ty, Tz, Q11, Q22, Q33, Q12, Q13, Q23, B1, B2, B3, hx, hy, hz):
    t3 = compute_three_point_update(Tx, Ty, Tz, Q11, Q22, Q33, Q12, Q13, Q23, B1, B2, B3, hx, hy, hz)
...
def sweep_axis_z(T, Q, B, source_mask, hx, hy, hz, direction):
    ...
    def scan_fn(T_prev, xs):
        ...
        T_new = voxel_update(T_curr, T_x, T_y, T_z, q11, q22, q33, q12, q13, q23, b1, b2, b3, hx, hy, hz)
...
def sweep_all(T, Q, B, source_mask, hx, hy, hz):
    ...
    Q_y = jnp.stack([Q[0], Q[2], Q[1], Q[4], Q[3], Q[5]], axis=0).transpose((0, 1, 3, 2))
    ...
    Q_x = jnp.stack([Q[2], Q[1], Q[0], Q[5], Q[4], Q[3]], axis=0).transpose((0, 3, 2, 1))
...
class VolumetricEikonalSolver(eqx.Module):
    ...
    def solve(self, metric: FinslerMetric, source_coords: jax.Array, 
              ...
              grid_shape: Tuple[int, int, int]) -> Tuple[jax.Array, jax.Array, jax.Array]:
        """
        ...
        Q_comp = jnp.stack([Q_mat[..., 0, 0], Q_mat[..., 1, 1], Q_mat[..., 2, 2],
                            Q_mat[..., 0, 1], Q_mat[..., 0, 2], Q_mat[..., 1, 2]], axis=0)
        ...
        T = _volumetric_solve(Q_comp, B_comp, source_mask, hx, hy, hz, self.max_iters, self.tol)
