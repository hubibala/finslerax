...
class GeodesicLearningSolver(eqx.Module):
    ...
    def solve(
        ...
        if train_mode:
            (final_inner, _), _ = jax.lax.scan(step_fn, (init_inner, opt_state), None, length=self.iterations)
        else:
            (final_inner, _), _ = jax.lax.scan(step_fn, (init_inner, opt_state), None, length=self.iterations)
            
        full_xs = jnp.concatenate([p_start[None, :], final_inner, p_end[None, :]], axis=0)
        full_vs = jax.vmap(metric.manifold.log_map)(full_xs[:-1], full_xs[1:])
