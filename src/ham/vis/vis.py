...
def plot_vector_field(
    ax, points, vectors, scale=0.2, color="blue", alpha=0.5, label=None, **kwargs
):
...
def plot_trajectory(ax, traj, color="red", label=None, **kwargs):
    ...
    # Pass kwargs (alpha, linewidth, linestyle) to plot
    ax.plot(xs[:, 0], xs[:, 1], xs[:, 2], color=color, label=label, **kwargs)
...
def plot_indicatrices(
    ax, metric, points, scale=0.15, n_theta=40, color="purple", alpha=0.8, **kwargs
):
    if len(points) == 0:
        return
    for p in points:
...
def generate_icosphere(radius=1.0, subdivisions=3):
    ...
    for _ in range(subdivisions):
        ...
        def get_midpoint(i1, i2):
            ...
            if key in midpoint_cache:  # noqa: B023
                return midpoint_cache[key]  # noqa: B023
            v1, v2 = np.array(new_verts[i1]), np.array(new_verts[i2])  # noqa: B023
            new_verts.append(((v1 + v2) / 2.0).tolist())  # noqa: B023
            return midpoint_cache.setdefault(key, len(new_verts) - 1)  # noqa: B023
