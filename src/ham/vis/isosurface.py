...
def differentiable_marching_cubes(
    ...
    # edges[..., safe_indices, :]
    batch_indices = jnp.expand_dims(safe_indices, axis=-1)
...
def compute_analytical_normals(triangles: jax.Array, volume: jax.Array, grid_extent: Tuple[float, float, float, float, float, float]):
    """
    ...
    grad_field = jnp.stack([grad_x, grad_y, grad_z], axis=-1)
    ...
    def interpolate_channel(chan):
        return jax.scipy.ndimage.map_coordinates(chan, grid_coords, order=1, mode='nearest')
