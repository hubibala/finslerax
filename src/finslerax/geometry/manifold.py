"""
Abstract Manifold base class — the topological domain for finslerax.
...
those are the responsibility of :class:`~finslerax.geometry.metric.FinslerMetric`.
...
for better accuracy (see :mod:`finslerax.geometry.manifolds`).
...
from finslerax.utils.math import GRAD_EPS, safe_norm
...
def _safe_norm_ratio_jvp(primals, tangents):
    ...
    # d(||x||) = x . x_dot / ||x||
    dnx = jnp.where(
        nx < zero_threshold, 0.0, jnp.sum(x * x_dot, axis=-1, keepdims=True) / nx
    )
...
class Manifold(eqx.Module, ABC):
    ...
    See `finslerax.geometry.manifolds` for concrete implementations (Sphere, Torus,
    ...
    def tangent_dot(self, x: jax.Array, u: jax.Array, v: jax.Array) -> jax.Array:
        ...
        an indefinite metric (e.g. :class:`~finslerax.geometry.manifolds.hyperboloid.Hyperboloid`,
