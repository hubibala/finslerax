...
from finslerax.utils.math import GRAD_EPS, NORM_EPS, safe_norm


def get_stream_function_flow(
    psi_fn: Callable[[jax.Array], jax.Array],
...
def rossby_haurwitz(
    R: int = 4, omega: float = 1.0, K: float = 1.0
) -> Callable[[jax.Array], jax.Array]:
...
def lamb_oseen_vortex(
    center: jax.Array, core_radius: float = 1.0, circulation: float = 1.0
) -> Callable[[jax.Array], jax.Array]:
    ...
    def flow(x: jax.Array) -> jax.Array:
        ...
        v_theta = (circulation / (2 * jnp.pi * r)) * (
            1.0 - jnp.exp(-r_sq / (core_radius**2))
        )
