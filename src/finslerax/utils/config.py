"""Global numerical-precision configuration for the finslerax library.

This module is the **single source of truth** for the floating-point precision
used throughout finslerax (core, examples, tests). finslerax follows the
JAX-native convention: precision is governed by JAX's own ``jax_enable_x64``
flag, and finslerax merely *reads* it.
...
class EpsKind(str, Enum):
    ...
    GRAD = "grad"  # :data:`finslerax.utils.math.GRAD_EPS` — sqrt backward guard.
