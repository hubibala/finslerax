"""A small, consistent visualization style for the finslerax example notebooks.
...
def plotly_sphere(radius=1.0, color=None, opacity=0.16, n=60):
    ...
    return go.Surface(
        ...
        showscale=False,
        hoverinfo="skip",
        lighting={"ambient": 0.9, "diffuse": 0.2, "specular": 0.05},
        name="",
    )
...
def plotly_cones(
    ...
    return go.Cone(
        ...
        name=name,
        hoverinfo="skip",
        colorbar={"title": colorbar_title, "thickness": 14, "len": 0.6}
        if showscale
        else None,
    )
