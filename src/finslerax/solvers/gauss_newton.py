...
Where :class:`~finslerax.solvers.avbd.AVBDSolver` relaxes the discrete path with
...
implicit adjoint in :mod:`finslerax.solvers.avbd` already uses for the backward pass.
...
* Designed for the latent-space use case — a :class:`~finslerax.geometry.manifolds.EuclideanSpace`
...
* Newton is **locally** convergent: from a cold straight-line guess that dives
  ...
  (:func:`finslerax.solvers.continuation.solve_continuation`) — anneal the metric and/or
...
from finslerax.solvers.avbd import Trajectory
...
class GaussNewtonGeodesic(eqx.Module):
    ...
    def solve(
        ...
        Returns:
            A :class:`~finslerax.solvers.avbd.Trajectory`.
        """
        n_inner = n_steps - 1
