...
See Also:
    finslerax.geometry.zoo.randers : Analytical Randers metric.
    finslerax.models.learned       : NeuralRanders (position-conditioned variant).
"""
...
from finslerax.utils.math import GRAD_EPS, WIND_STIFFNESS, causal_wind_scale
...
def project_b_norm(
    ...
    smooth, identity-preserving causal clamp :func:`finslerax.utils.causal_wind_scale`:
    ...
    Args:
        ...
        stiffness: Sharpness of the smooth clamp. Defaults to
            :const:`finslerax.utils.WIND_STIFFNESS`.
...
class CovariateConditionedRanders(AsymmetricMetric):
    ...
    * ``local_cnn``:  a fully-convolutional encoder maps terrain rasters
    ...
    structure to :class:`finslerax.geometry.zoo.Randers`).
    ...
    def zermelo_data(self, x: jax.Array) -> tuple[jax.Array, jax.Array, jax.Array]:
        ...
        Implements the :class:`~finslerax.geometry.metric.AsymmetricMetric` interface
    ...
    def metric_fn(self, x: jax.Array, v: jax.Array) -> jax.Array:
        ...
        :meth:`finslerax.geometry.zoo.Randers.metric_fn`, with ``G`` and ``b``
