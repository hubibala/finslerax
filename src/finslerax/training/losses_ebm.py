...
def sgld_step(x, ebm_model, step_size, noise_scale, key):
    ...
    score_clipped = jnp.where(
        score_norm > 10.0, score * (10.0 / (score_norm + 1e-8)), score
    )
...
class ContrastiveDivergenceLoss(LossComponent[EnergyModel]):
    ...
    def __call__(self, model: EnergyModel, batch: tuple, key: jax.Array) -> jnp.ndarray:
        # In TrainingPipeline, vmap is applied over the batch dimension,
...
class DenoisingScoreMatchingLoss(LossComponent[EnergyModel]):
...
class MSELoss(LossComponent[EnergyModel]):
    ...
    def __call__(self, model: EnergyModel, batch: tuple, key: jax.Array) -> jnp.ndarray:
        inputs = batch[0]
        # In TrainingPipeline, batch is (X, V, labels). The target scalar is in labels.
        target = batch[2]
