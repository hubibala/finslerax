"""Multi-phase declarative training pipeline for finslerax models.

Provides TrainingPhase (a declarative description of one training stage) and
TrainingPipeline (an orchestrator that executes phases sequentially with per-phase
parameter freezing, modular loss composition, and pair/triple batching).

See also:
    spec/ARCH_SPEC.md § 6.4 -- Training Pipeline.
    finslerax.training.losses -- Modular loss components.
"""
...
@dataclass
class TrainingPhase:
    """Declarative description of one phase in the TrainingPipeline training loop.
...
class TrainingPipeline:
    """Orchestrates finslerax model training through sequential declarative phases.
    ...
    See also:
        spec/ARCH_SPEC.md § 6.4, finslerax.training.losses.
    """
    ...
    def fit(
        ...
        Args:
            ...
            device: Target device for training data. ``"cpu"`` (default) or
                ...
                ``finslerax.utils.configure_device()`` at script startup for a
                global alternative.
        ...
        from finslerax.utils.device import get_device
        ...
        for phase in phases:
            ...
            phase_losses = phase.losses
            phase_optimizer = phase.optimizer
            ...
            def loss_fn(diff, static, batch_data, step_key):
                ...
                total_loss = 0.0
                stats = {}
                for loss_comp in phase_losses:  # noqa: B023
            ...
            def train_step(diff, static, state, batch_data, step_key):
                ...
                updates, new_state = phase_optimizer.update(grads, state, diff)  # noqa: B023
            ...
            for epoch in range(phase.epochs):
                ...
                if epoch % 10 == 0 or epoch == phase.epochs - 1:
                    stat_str = " | ".join(
                        [
                            f"{k}: {v / steps_per_epoch:.4f}"
                            for k, v in epoch_stats.items()
                        ]
                    )
                    print(
                        f"Epoch {epoch:03d} | Total Loss: {epoch_loss / steps_per_epoch:.4f} | {stat_str}"
                    )
